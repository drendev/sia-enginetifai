...
export function DeliveryGrowth() {
    const { Option } = Select;
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                const response = await fetch('/api/delivery/deliverygrowth', {
                    method: 'POST'
                });
    ...
    return (
        <>
            <div className="flex justify-between">
                <h1 className='text-red-900 font-sans font-bold text-xl pb-2'>Delivery Growth</h1>
                <Select
                    defaultValue="daily"
