...
export function PendingDeliveriesCourier() {
    const [deliveries, setDelivery] = useState([] as PendingDeliveries[]);
    ...
    useEffect(() => {
        const fetchActiveDeliveryData = async () => {
            const res = await fetch('/api/delivery/pendingdeliveries', {
                method: 'POST',
                headers: {
            ...
            const sortedData = data.sort((a: PendingDeliveries, b: PendingDeliveries) => 
    ...
    return(
        <>
            <div className="w-[28rem]">
                <h1 className='text-red-900 font-sans font-bold text-xl pb-2'>Pending Deliveries</h1>
